...
<script>
import {IsPosInteNum} from 'common/js/utils.js'
export default {
  name: 'Pager',
  props: {
    curPage: Number,
  ...
  methods: {
    ...
    // 输入框跳转
    goSp (event) {
      if (!IsPosInteNum(document.getElementById('goPageInp').value)) {
        this.$message({
          type: 'error',
          message: '请输入正整数',
          duration: 1500
  ...
  computed: {
    pages () {
      // 基准页面，至少为5页，取奇数
      const defaultPage = 7 // 5 7 9 11之类都可以
      const delta = (defaultPage - 5) / 2
      ...
      if (!this.lEls && this.rEls) { // !1234 5 ... total
        ...
        for (let i = total - defaultPage + 3; i < total; i++) {
          arr.push(i)
        }
      } else { // !<=7
